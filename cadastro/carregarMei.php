<?php
function carregarMei($uuid) {
    $pasta_destino = "../image/mei/";
    $caminho_destino = $pasta_destino . basename($_FILES["mei"]["name"]);
    $uploadOk = 1;
...
// Verifique se o arquivo de imagem é uma imagem real ou uma imagem falsa
    if(isset($_POST["confirmarCadastro"])) {
        $check = getimagesize($_FILES["mei"]["tmp_name"]);
        if($check !== false) {
...
// se tudo estiver ok, tente fazer o upload do arquivo
        ...
        if (move_uploaded_file($_FILES["mei"]["tmp_name"], $caminho_destino)) {
// echo "O arquivo". basename ($ _FILES ["mei"] ["nome"]). "foi carregado";
